...
class CustomerController extends Controller
{
    ...
    public function forgot_verify(Request $request){
        ...
        if(!$customer_info){
            Toastr::error('আপনার ইমেইল পাওয়া যায়নি');
            return back();
        }
        
        $reset_code = rand(100000,999999);
        $customer_info->forgot = $reset_code;
        $customer_info->save();
        ...
        try {
            ...
            $subject = 'পাসওয়ার্ড রিসেট করুন - ' . ($site_setting->name ?? 'Shop');
            
            $message = "নমস্কার " . $customer_info->name . ",\n\n";
            $message .= "আপনার পাসওয়ার্ড রিসেট কোড: " . $reset_code . "\n\n";
            $message .= "এই কোডটি ৩০ মিনিটের জন্য বৈধ।\n\n";
            $message .= "যদি আপনি এই অনুরোধ না করেন, তাহলে এই ইমেইল উপেক্ষা করুন।\n\n";
            $message .= "ধন্যবাদ,\n";
            $message .= ($site_setting->name ?? 'Shop') . " টিম";
            
            $headers = "From: " . (env('MAIL_FROM_ADDRESS', 'noreply@shop.com')) . "\r\n";
            $headers .= "Content-Type: text/plain; charset=UTF-8\r\n";
            
            // Using PHP's mail function
            if(mail($to, $subject, $message, $headers)) {
                session::put('verify_email', $request->email);
                Toastr::success('আপনার ইমেইলে রিসেট কোড পাঠানো হয়েছে');
                return redirect()->route('customer.forgot.reset');
            } else {
                throw new \Exception('mail() সিস্টেম কল ব্যর্থ হয়েছে');
            }
        } catch (\Exception $e) {
            Log::error('Password reset email sending failed: ' . $e->getMessage());
            Toastr::error('ইমেইল পাঠাতে ব্যর্থ, পুনরায় চেষ্টা করুন');
            return back();
        }
    }
    ...
    public function forgot_resend(Request $request){
        ...
        if(!$customer_info) {
            Toastr::error('গ্রাহক খুঁজে পাওয়া যায়নি');
        ...
        try {
            ...
            $message = "নমস্কার " . $customer_info->name . ",\n\n";
            $message .= "আপনার নতুন পাসওয়ার্ড রিসেট কোড: " . $reset_code . "\n\n";
            $message .= "এই কোডটি ৩০ মিনিটের জন্য বৈধ।\n\n";
            $message .= "ধন্যবাদ,\n";
            $message .= ($site_setting->name ?? 'Shop') . " টিম";
            ...
            // Using PHP's mail function
            if(mail($to, $subject, $message, $headers)) {
                Toastr::success('রিসেট কোড পুনরায় পাঠানো হয়েছে');
                return redirect()->back();
            } else {
                throw new \Exception('mail() সিস্টেম কল ব্যর্থ হয়েছে');
            }
        } catch (\Exception $e) {
            Log::error('Password reset email resend failed: ' . $e->getMessage());
            Toastr::error('ইমেইল পাঠাতে ব্যর্থ, পুনরায় চেষ্টা করুন');
            return back();
        }
    }
    public function forgot_reset(){
        if(!Session::get('verify_email')){
          Toastr::error('কিছু ভুল হয়েছে, পুনরায় চেষ্টা করুন');
          return redirect()->route('customer.forgot.password'); 
        };
    ...
    public function forgot_store(Request $request){
        ...
        if($customer_info->forgot != $request->code){
            Toastr::error('আপনার রিসেট কোড সঠিক নয়');
            return redirect()->back();
        }
        ...
        if(Auth::guard('customer')->attempt(['email' => $customer_info->email, 'password' => $request->password])) {
            ...
            Toastr::success('পাসওয়ার্ড সফলভাবে আপডেট করা হয়েছে', 'সাফল্য!');
            return redirect()->intended('customer/account');
        }
        
        Toastr::error('লগইনে ব্যর্থ, পুনরায় চেষ্টা করুন');
        return back();
    }
